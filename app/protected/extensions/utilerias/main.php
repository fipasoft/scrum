...
class Utils {
	...
	static function verificaCampos($prefijo,$campos){
		foreach($campos as $c){
			if(trim($_POST[$prefijo][$c])==""){
				return false;
			}
		}
		return true;
	...
	/*
	 * Restar horas( horas con minutos )
	 */
	function restaHorasMinutos($horaIni, $horaFin){
		return (date("H:i", strtotime("00:00") + strtotime($horaFin) - strtotime($horaIni) ));
	}
	/*
	 * Restar horas( horas con minutos y segundos)
	 */
	function restaHorasMinutosSegundos($horaIni, $horaFin){
		return (date("H:i:s", strtotime("00:00:00") + strtotime($horaFin) - strtotime($horaIni) ));
	}
	
	/*
	 * Sumar horas(horas con minutos y con segundos)
	 * sumar("05:20:00","05:20:30");
	 */
	function sumarHorasMinutosSegundos($h1,$h2){
	 ...
	 * segundos2horas(3721); devuelve 01:02:01
	 */
	function segundos2horas($seg_ini) {
	 ...
	 * minutos2horas(70); devuelve 01:10:00
	 */
	function minutos2horas($minutos){
		$seg = intval($minutos) * 60;
		
		return Utils::segundos2horas($seg);
	}
